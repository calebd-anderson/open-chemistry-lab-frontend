// Test script to verify grid position calculations for key elements using the actual implementation
console.log("Testing periodic table grid position calculations with correct expectations...\n");
...
// Test cases for verification - correcting my previous assumptions
const testCases = [
...
// Let's also test some additional elements to make sure the logic is sound
console.log("\nAdditional verification tests:");
const additionalTests = [
  { atomicNumber: 21, expected: { row: 4, col: 3 }, description: "Scandium (should be in period 4, column 3)" },
  { atomicNumber: 39, expected: { row: 5, col: 3 }, description: "Yttrium (should be in period 5, column 3)" },
  { atomicNumber: 57, expected: { row: 8, col: 4 }, description: "Lanthanum (should be in period 8, column 4)" },
  { atomicNumber: 89, expected: { row: 9, col: 4 }, description: "Actinium (should be in period 9, column 4)" }
];
...
additionalTests.forEach(testCase => {
  ...
  if (isCorrect) {
    console.log(`✅ ${testCase.description}: row=${result.row}, col=${result.col}`);
  } else {
    console.log(`❌ ${testCase.description}: expected row=${testCase.expected.row}, col=${testCase.expected.col}, got row=${result.row}, col=${result.col}`);
  }
});